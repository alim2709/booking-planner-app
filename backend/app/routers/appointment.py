...
async def create_appointment(data: CreateAppointmentModel, user_details: dict = Depends(access_token_service)):
    try:
...
async def update_appointment(appointment_id: int, data: CreateAppointmentModel, user_details: dict = Depends(access_token_service)):
    try:
